...
- 修复 `config.ts` 默认 systemPrompt 里残留的 "Digital IDE" 文案
- 工具调用在 Vivado busy 时的显式提示

### 6.2 Agent 化演进（Codex 式"能动手的智能体"，差异化方向）

SynthAid-IDE **保持 VSCode 插件形态**（VSCode 本身就是桌面环境），不另做独立桌面 App，
而是把 AI 从"只读问答"演进为"能改代码、能跑工具、能迭代修复的 FPGA 智能体"：

```
Phase 1（现状）：读
  · 问设计 → RTL 上下文直接答
  · 查 Vivado → run_vivado_tcl 工具

Phase 2：写（新增"写"类工具）
  · edit_file / new_file：改/建 RTL（WorkspaceEdit）
  · run_build / run_sim：复用现有 iverilog / Vivado 命令基础设施
  · read_output：读回错误 → 复用日志分析

Phase 3：agent 循环（plan → apply → verify）
  · 如"给顶层加 UART" → 规划 → 写代码 → 跑仿真 → 看报错 → 迭代修复 → 汇报
```

要点：
- **模型无关**：工具循环基于 function calling，任何 OpenAI 兼容模型（DeepSeek/OpenAI/Qwen…）均可驱动，不绑定 Codex 的模型/App。
- **安全边界**：自主改 RTL 必须做 **plan/apply 确认 + diff 展示 + 可撤销 + 命令沙箱**（禁破坏性命令）；写工具仅限 `user/` 下文件。
